...
# 导入RequestSend类
from utils.rquestsutil import RequestSend
...
class TestApi:
    ...
    def test_run(self, case):
        ...
        # 根据关联获取headers参数中是否有变量需要被替换，有则替换，无则默认
        headers = self.correlation(headers)
        # 根据关联获取cookies参数中是否有变量需要被替换，有则替换，五则默认
        cookies = self.correlation(cookies)
        # 根据关联获取data参数中是否有变量需要被替换，有则替换，无则默默人
        ...
        try:
            ...
            # 打印日志
            logger.info("执行用例成功，请求的结果为{}".format(res_data))
        # 异常捕获
        except:
            # 打印日志
    ...
    def assert_respose(self, case, res_data):
        ...
        # 无论是否出现异常，都执行下面的代码
            ...
            # 根据结果是True还是False进行断言验证，成功则通过，失败则未通过
            assert is_pass
        # 返回变量结果
        return is_pass
